...
app.get('/', (req, res) => {
    ...
    get_data("all", []).then((resolve) => {
        for(i=0;i<resolve.length;i++){
            console.log(resolve)//#well,#nobad// {{resolve[0].id}}
            l.push(resolve[i]["tag"].split(","))
        }
        console.log(l)
        // res.send("hello")
...
app.get('/search', (request, response) => {
    ...
    get_data("search", []).then((resolve) => {
        ...
        for (let i = 0, l = resolve.length; i < l; i++) {
            var obj = resolve[i];
            var similarity = stringSimilarity.compareTwoStrings(String(search.toLowerCase()), obj.category.toLowerCase())
            if (similarity >= 0.2) {
                console.log(obj.name, 'Сходство(name):', similarity)
                SearchedList.push(obj)
            }
